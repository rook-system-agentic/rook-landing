...
export function buildContactProperties(lead) {
  ...
  const properties={
    ...
    nome_do_restaurante:lead.company,
    cidade:lead.city.name,
    uf:lead.city.uf,
    segmento:ASAFLOW_SEGMENTS[lead.segment]||'Outro',
  };
...
export function buildDealProperties(lead) {
  const attribution=normalizedAttribution(lead);
  const properties={origem:acquisitionOrigin(lead),empresa_cliente:lead.company};
