...
test('resultado financeiro recebido do cliente é recalculado',()=>{
 const parsed=validateAcquisition({...lead,segment:'a_la_carte',period:'2026-08',simulation:{tool:'cmv',revenueBasis:'net',revenue:100000,cmvPercent:38,segment:'a_la_carte',result:{monthlyDifference:999999}}},cities);
 assert.equal(parsed.value.simulation.result.monthlyDifference,6000);
});
test('cadastro não aceita benchmark de um segmento diferente do estabelecimento',()=>{
 const parsed=validateAcquisition({...lead,period:'2026-08',simulation:{tool:'cmv',revenueBasis:'net',revenue:100000,cmvPercent:38,segment:'a_la_carte'}},cities);
 assert.equal(parsed.ok,false);
 assert.match(parsed.errors.simulation,/segmento/);
});
